...
def Differentiate(symbols: str, fx: str) -> str:
...
def MarginalUtility(symbols: str, fx: str, variable: str) -> str:
    symbols = ValidateSymbols(symbols)
...
def main() -> int:
    ...
    derivative = Differentiate('x', '(' + fx + ') * (' + gx + ')')
    print(derivative)
    ...
    derivative = MarginalUtility('x y z', fx, 'x')
    print(derivative)
    
    # Partial derivative with respect to y: x**2*z**5
    derivative = MarginalUtility('x y z', fx, 'y')
    print(derivative)
    
    # Partial derivative with respect to z: 5*x**2*y*z**4
    derivative = MarginalUtility('x y z', fx, 'z')
    print(derivative)
    ...
    MUp = MarginalUtility('p c', fx, 'p')
    print(MUp)
    
    # Differentiate f(x) with respect to c: sqrt(c*p)/(2*c)
    MUc = MarginalUtility('p c', fx, 'c')
    print(MUc)
